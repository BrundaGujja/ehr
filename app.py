...
#Mongodb setup
client = pymongo.MongoClient("mongodb+srv://Brunda:Brunda123@cluster0.ccfm2.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
def create_patient():
    ...
    passwd = encrypt_password(request.form['pwd'])
    ...
    # Find the last inserted document based on the numeric part of _id
    last_doc_cursor = mycol.find({"_id": {"$regex": "^PAT[0-9]{4}$"}}).sort('_id', -1).limit(1)

    last_doc = None
    try:
        last_doc = last_doc_cursor.next()  # Attempt to get the last document
        last_id_num = int(last_doc['_id'][3:])  # Extract the numeric part from _id
        new_id_num = last_id_num + 1
    except StopIteration:
        # If no document is found, start from 1
        new_id_num = 1

    # Create a new patient ID
    new_id = f'PAT{new_id_num:04}'

    # Create the patient block
    block = {
        '_id': new_id,
        'timestamp': st,
        'first': first,
        ...
        'state': state,
        'aadhar': aadhar,
        'prevhash': last_doc['hash'] if last_doc else "0"  # Safely use last_doc here
    }

    # Calculate hash for the block
    block_string = json.dumps(block, sort_keys=True)
    hashval = hashlib.sha256(block_string.encode()).hexdigest()
    block['record'] = f'{new_id}REC'
    block['hash'] = hashval
    ...
    # Insert patient and record
    try:
        mycol.insert_one(block)
        myrow.insert_one(rec)
    except pymongo.errors.DuplicateKeyError:
        return "Duplicate Key Error. Please try again.", 500
    except pymongo.errors.PyMongoError as e:
        # Catching all other PyMongo exceptions
        return f"An error occurred while writing to MongoDB: {str(e)}", 500

    session['user'] = new_id
    blocks = list(mycol.find())
    return render_template('patcreate.html', posts=blocks)
...
clienth = pymongo.MongoClient("mongodb+srv://Padmavathi:Padma123@cluster0.701g6.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
def result():
    ...
    # Assign the new ID to the block
    block['_id'] = new_id

    try:
        mycolh.insert_one(block)
    except pymongo.errors.DuplicateKeyError:
        return f"Duplicate Key Error: A document with _id '{new_id}' already exists.", 500
    except pymongo.errors.PyMongoError as e:
        return f"An error occurred while writing to MongoDB: {str(e)}", 500

    # Further processing...
    return "Block inserted successfully", 200

def generate_unique_id():
    last_doc = mycolh.find().sort('_id', -1).limit(1)
    try:
        last_id = last_doc.next()['_id']
        new_id_num = int(last_id[4:]) + 1
    except StopIteration:
        new_id_num = 1

    new_id = f'POST{new_id_num:03}'  # Generates POST001, POST002, etc.
    return new_id
